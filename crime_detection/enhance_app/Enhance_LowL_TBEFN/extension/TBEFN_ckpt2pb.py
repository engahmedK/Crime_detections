...
def simple_unet(input,name):
    ...
    with tf.variable_scope(name):
        ...
        conv8 = slim.conv2d(conv7, 3, [3, 3], rate=1, activation_fn=tf.nn.relu, scope='u_conv8')
...
constant_graph = graph_util.convert_variables_to_constants(sess, sess.graph_def, ['fusion/fusion_3/BiasAdd'])

"""
inp = cv2.imread('./input_dir/03.bmp')/255.
inp = inp.reshape(1, inp.shape[0], inp.shape[1], inp.shape[2])
feed_dict = {in_image: inp}
[out] = sess.run([uf_out], feed_dict=feed_dict)
out = np.array(out[0])
cv2.imwrite('./ckpt2pb.png', out*255.)
print(out)
"""

# write the pb file
os.makedirs('../Pb_File', exist_ok=True)
with tf.gfile.FastGFile('../Pb_File/TBEFN.pb', mode='wb') as f:
